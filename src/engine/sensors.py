...
def poll_all_sensors() -> gpd.GeoDataFrame:
    ...
    measurements = (
        poll_essential_edinburgh()
        + poll_edintraveldata(
            [sensor_dict for sensor_dict in sensor_descriptions.to_dict(orient="records")
             if sensor_dict['type'] == SensorType.CEC_PED_FLUX_COUNTER]))

    # tabulate measurements
    measurements = pd.DataFrame([
        {**asdict(m), "measurement_class": m.__class__.__name__}
    ...
    measurements.loc[is_ped_flux, "density_pax_per_m2"] = (
        measurements.loc[is_ped_flux, 'flow_pax_per_hour'] / 3600 
        / measurements.loc[is_ped_flux, 'measurement_width_m'] / AVERAGE_WALKING_SPEED_MPS)
