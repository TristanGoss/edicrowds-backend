...
from engine.config import CACHE_ROOT
...
class SimpleCache:
    ...
    def read(self) -> Optional[Union[dict, list]]:
        ...
        for path in self.cache_root.glob(f"{self.file_prefix}*.json"):
            ...
            cached_nowcast_dt = self.from_os_safe_iso_timestamp(
                path.stem.replace(self.file_prefix, ''))
    ...
    def write(self, data: Union[dict, list]) -> None:
        ...
        with open(self.cache_root / f"{self.file_prefix}{self.to_os_safe_iso_timestamp(datetime.now())}.json", "w") as fh:
            json.dump(data, fh)
